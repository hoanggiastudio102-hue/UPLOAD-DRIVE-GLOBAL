...
def add_reports(broker, devices):
    ...
    summary = dict(employees=len({d['employee_code'] for d in active if d['employee_code']}),
        ...
        attention=sum(bool(d.get('alert')) and not d['alert']['acknowledged'] for d in active),
        acknowledged=sum(bool(d.get('alert')) and bool(d['alert']['acknowledged']) for d in active),
